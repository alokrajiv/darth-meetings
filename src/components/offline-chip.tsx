...
import { CloudOff, Loader2, Settings, Wifi } from 'lucide-react';
 ...
 *  - otherwise nothing (the header stays clean for the 99% case).
 *
...
export function OfflineChip() {
  const { mode, online, syncing, syncState, pins, storage, exitOffline } = useOffline();
  ...
  return (
    <div className="relative" ref={ref} data-offline-chip>
      <button
        ...
        onClick={() => setOpen((o) => !o)}
        title={offline ? 'Offline mode — browsing what is saved on this device' : 'Saving meetings for offline use'}
        className={`inline-flex h-7 items-center gap-1.5 rounded-full border px-2.5 text-xs font-medium transition-colors ${
          offline
            ? 'border-amber-300 bg-amber-50 text-amber-900 hover:bg-amber-100 dark:border-amber-700/60 dark:bg-amber-950/60 dark:text-amber-200 dark:hover:bg-amber-900/60'
      ...
      {open && (
        ...
        >
          <p className="text-sm font-medium">
            {offline ? 'Offline mode' : 'Saving for offline'}
          </p>
          {!offline && (
