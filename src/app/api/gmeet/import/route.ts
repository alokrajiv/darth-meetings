...
import type { MeetParticipantInfo, SpeakerLabel } from '@/lib/format';
import {
  GoogleApiError,
  captureMeetActuals,
...
function humanizeLocalPart(email: string): string {
 ...
 */
async function registerPeopleFromMeeting(
  candidates: Array<{ email: string; name?: string | null }>,
  createdBy: string
): Promise<void> {
  ...
  for (const c of candidates) {
    ...
    try {
      ...
      console.warn('[gmeet/import] people registration failed for', email, err);
    }
  }
}
...
/**
 * Meet transcripts come with REAL speaker names — map every speaker
 * immediately (so the panel never shows "Unnamed · <name>") and register
 * anyone we can resolve an email for into the people directory. Never
 * clobbers mappings a user already made (force re-imports).
 */
async function autoNameSpeakers(
  ...
  const labels: SpeakerLabel[] = speakerNames.map((name) => {
    ...
    return {
      ...
      customName: name,
      description: email
        ? `${email} · auto-matched from Google Meet`
        : 'auto-added from Google Meet',
    };
  });
